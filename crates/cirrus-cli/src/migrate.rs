//! `cirrus migrate` — state-dir version migration scaffolding.
//!
//! cirrus persists almost no on-disk state by default (RunStart/Stop
//! flow through Documents to Tiled / Kafka / files via sinks; the
//! engine itself is in-memory). The pieces that DO live on disk:
//!
//! - `~/.cirrus/profiles/*.toml` — saved Tiled / qs connection
//!   profiles (tiled-rs's `tiled-client::profiles` mechanism).
//! - `~/.cirrus/runs.jsonl` — JsonlSink output, one Document per line.
//! - `~/.cirrus/tokens/*` — cached OAuth tokens for tiled-client.
//!
//! When cirrus's on-disk format changes between versions, this
...
/// CLI arguments for `cirrus migrate`.
#[derive(Args, Debug)]
pub struct MigrateArgs {
    /// State directory. Defaults to `$XDG_CONFIG_HOME/cirrus` (or
    /// `~/.cirrus` if XDG is unset).
    #[arg(long)]
    ...
    /// Target schema version. Default = current cirrus version.
...
pub fn run(args: MigrateArgs) -> i32 {
    ...
    println!("cirrus migrate: state_dir = {}", dir.display());
    ...
    if inventory.is_empty() {
        println!("  (no recognizable cirrus state files)");
        return 0;
    }
...
fn default_state_dir() -> PathBuf {
    ...
    if let Ok(home) = std::env::var("HOME") {
        return PathBuf::from(home).join(".cirrus");
    }
    PathBuf::from(".cirrus")
}
