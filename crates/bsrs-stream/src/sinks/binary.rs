    ...
    async fn ensure_open(&self) -> Result<()> {
        ...
        if guard.is_none() {
            let mut f = OpenOptions::new()
                ...
                .open(&self.path)
                .await
                .map_err(|e| BsrsError::Backend(format!("binary sink open: {e}")))?;
            f.write_all(MAGIC)
                .await
                .map_err(|e| BsrsError::Backend(format!("binary sink write magic: {e}")))?;
            *guard = Some(f);
        }
        Ok(())
    }
...
impl FrameSink for BinaryFrameSink {
    async fn accept(&self, frame: Frame) -> Result<()> {
        ...
        let f = guard
            .as_mut()
            .ok_or_else(|| BsrsError::State("binary sink not open".into()))?;
        ...
        f.write_all(&len)
            .await
            .map_err(|e| BsrsError::Backend(format!("binary sink write len: {e}")))?;
        f.write_all(&frame.payload)
            .await
            .map_err(|e| BsrsError::Backend(format!("binary sink write payload: {e}")))?;
...
impl DetectorWriter for BinaryFrameSink {
    ...
    async fn close(&self) -> Result<()> {
        ...
        if let Some(mut f) = guard.take() {
            f.flush()
                .await
                .map_err(|e| BsrsError::Backend(format!("binary sink flush: {e}")))?;
        }
        Ok(())
    }
}
