...
use bsrs_event_model::{DataKey, Dtype, StreamDatum, StreamRange, StreamResource};
use bsrs_protocols_async::{DetectorWriter, Frame, FrameSink, StreamAsset};
...
impl Hdf5FrameSink {
    ...
    fn send(&self, cmd: Cmd) -> Result<()> {
        ...
        let tx = g
            .as_ref()
            .ok_or_else(|| BsrsError::Backend("hdf5 worker shut down".into()))?;
        tx.send(cmd)
            .map_err(|_| BsrsError::Backend("hdf5 worker dropped".into()))
    }
    ...
    async fn ensure_open(&self) -> Result<()> {
        ...
        let res = rep_rx
            .await
            .map_err(|_| BsrsError::Backend("hdf5 open: worker dropped reply".into()))?;
        if res.is_ok() {
...
impl FrameSink for Hdf5FrameSink {
    async fn accept(&self, frame: Frame) -> Result<()> {
        ...
        rep_rx
            .await
            .map_err(|_| BsrsError::Backend("hdf5 append: worker dropped reply".into()))??;
        ...
        match cmd {
            ...
            Cmd::Append { payload, reply } => match state.as_ref() {
                Some((_, ds)) => {
                    let res = ds
                        .append::<u8>(&payload)
                        .map_err(|e| BsrsError::Backend(format!("hdf5 append: {e}")));
                    let _ = reply.send(res);
                }
                None => {
                    let _ = reply.send(Err(BsrsError::State("hdf5 sink not open".into())));
                }
            },
        }
    }
...
fn open_file(
    ...
    let entry = file
        .create_group("entry")
        .map_err(|e| BsrsError::Backend(format!("hdf5 sink: create entry: {e}")))?;
    let instr = entry
