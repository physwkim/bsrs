...
pub async fn run(args: ManagerArgs) -> i32 {
    ...
    #[cfg(feature = "ca")]
    {
        ...
        for spec in &args.ca_motor {
            ...
            let m = match bsrs_host::ca_devices::CaMotor::connect_async(name, val_pv, rbv_pv).await
            {
                Ok(m) => m,
                Err(e) => {
        ...
        for spec in &args.ca_detector {
            ...
            let d = match bsrs_host::ca_devices::CaDetector::connect_async(name, pv).await {
                Ok(d) => d,
                Err(e) => {
