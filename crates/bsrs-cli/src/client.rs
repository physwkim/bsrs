//! `bsrs qs <subcommand>` — REQ client for a running bsrs-qs server.
...
/// Top-level args for `bsrs qs`.
#[derive(Args, Debug)]
pub struct ClientArgs {
    /// Control REP socket address of the running bsrs-qs server.
...
enum Cmd {
    ...
    /// Attach to a running bsrs-qs daemon for an interactive Lua
    ...
    /// evaluator (the default `bsrs qs-manager` does this).
    Repl {
...
enum EnvCmd {
    ...
    Close,
    /// `environment_destroy` — force-drop without checks (bsrs aliases
    /// to `environment_close`).
    Destroy,
    /// `environment_update` — refresh registry (no-op in bsrs).
    Update,
}
...
enum AllowedCmd {
    /// `plans_allowed`.
    Plans,
    /// `plans_existing` — superset of plans_allowed (bsrs has no
...
fn dispatch(args: ClientArgs) -> Result<Value, String> {
    ...
    let resp = sock.recv_bytes(0).map_err(|e| {
        format!(
            "recv: {e} (server not responding within {} ms — start `bsrs qs-manager`?)",
            args.timeout_ms
        )
    })?;
