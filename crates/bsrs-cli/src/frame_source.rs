//! `bsrs frame-source` — D21 multi-process scaffold.
//!
...
//! ```text
//! ┌──────────────────────────┐  ZMQ Documents  ┌────────────────────┐
//! │  bsrs frame-source     │ ──────────────► │ bsrs repl / qs   │
...
//! `bsrs-stream::PvaMonitorSource` → `Hdf5FrameSink` →
//! `ZmqDocumentSink`.

use clap::Args;

/// CLI arguments for `bsrs frame-source`.
#[derive(Args, Debug)]
pub struct FrameSourceArgs {
    ...
    /// messages (e.g. `ipc:///tmp/bsrs-frames.sock` or
...
pub fn run(args: FrameSourceArgs) -> i32 {
    println!("bsrs frame-source — D21 multi-process scaffold");
    ...
    #[cfg(not(feature = "frame-source"))]
    {
        eprintln!(
            "\nbuild without `--features frame-source`: this subcommand validates args \
             and exits. Wire format is exercised in bsrs-callbacks (ZmqDocumentSource \
...
async fn run_wired(args: FrameSourceArgs) -> i32 {
    use bsrs_callbacks::{Serializer, ZmqDocumentSink};
    use bsrs_event_model::Document;
    use bsrs_protocols_async::{DetectorWriter, FrameSink, FrameSource, StreamAsset};
    use bsrs_stream::sinks::Hdf5FrameSink;
    use bsrs_stream::sources::PvaMonitorSource;
    use bsrs_stream::FramePipe;
    ...
    let doc_loop = tokio::spawn(async move {
        ...
        loop {
            ...
            while let Some(asset) = stream.next().await {
                ...
                if let Err(e) = bsrs_engine::DocumentSink::dispatch(&*doc_sink_clone, &doc).await {
