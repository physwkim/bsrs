...
use bsrs_host::lua_env::build_lua;
...
pub fn run(args: ReplArgs) -> i32 {
    ...
    let mut sinks: Vec<Arc<dyn bsrs_engine::DocumentSink>> = Vec::new();
    if let Some(addr) = &args.doc_zmq {
        match bsrs_callbacks::ZmqDocumentSink::bind(addr) {
            Ok(s) => {
                eprintln!("bsrs repl: publishing Documents on ZMQ {addr}");
                sinks.push(Arc::new(s) as Arc<dyn bsrs_engine::DocumentSink>);
            }
            Err(e) => {
    ...
    if let Some(path) = &args.doc_jsonl {
        match bsrs_core::runtime::bsrs_runtime().block_on(bsrs_callbacks::JsonlSink::open(path)) {
            Ok(s) => {
                eprintln!(
    ...
    if let Some(url) = &args.doc_tiled {
        let mut sink = match bsrs_callbacks::TiledSink::new(url, &args.doc_tiled_container) {
            Ok(s) => s,
            Err(e) => {
        ...
        sinks.push(Arc::new(sink) as Arc<dyn bsrs_engine::DocumentSink>);
    }
