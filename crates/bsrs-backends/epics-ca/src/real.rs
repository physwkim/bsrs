...
pub fn ca_context() -> Arc<CaContext> {
    ...
    let client_res = if tokio::runtime::Handle::try_current().is_ok() {
        std::thread::scope(|s| {
            s.spawn(|| bsrs_core::runtime::bsrs_runtime().block_on(CaClient::new()))
                .join()
                .expect("ca_context: bootstrap thread panicked")
        })
    } else {
        bsrs_core::runtime::bsrs_runtime().block_on(CaClient::new())
    };
...
impl<T: Clone + Send + Sync + 'static> bsrs_devices::BackendFromPv for EpicsCaBackend<T> {
...
impl bsrs_devices::BackendFromPv for CaEnumBackend {
    fn from_pv(pv: &str) -> Self {
        Self::new(pv)
    }
}
