...
use bsrs_devices::StandardDetector;
...
pub struct SoftDetectorWriter {
    ...
    compose: tokio::sync::Mutex<Option<Arc<bsrs_event_model::compose::RunBundle>>>,
}
...
impl SoftDetectorWriter {
    ...
    /// correct run UID.
    pub async fn bind_to_run(&self, compose: Arc<bsrs_event_model::compose::RunBundle>) {
...
impl DetectorWriter for SoftDetectorWriter {
    ...
    fn collect_stream_docs(&self, up_to: u64, descriptor: &str) -> BoxStream<'_, StreamAsset> {
        ...
        let resource_uid = {
            ...
            if let Some(u) = guard.clone() {
                ...
                *guard = Some(new_uid.clone());
                let resource = bsrs_event_model::StreamResource {
                    uid: new_uid.clone(),
        ...
        if up_to > last {
            let datum = bsrs_event_model::StreamDatum {
                uid: uuid::Uuid::new_v4().to_string(),
                stream_resource: resource_uid,
                descriptor: descriptor.to_string(),
                indices: bsrs_event_model::StreamRange {
                    start: last,
                    stop: up_to,
                },
                seq_nums: bsrs_event_model::StreamRange {
                    start: last + 1,
                    stop: up_to + 1,
                },
            };
