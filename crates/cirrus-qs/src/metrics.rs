//! Prometheus metrics endpoint for cirrus-qs.
//!
...
//! # Recorded metrics
//!
//! - `cirrus_qs_rpc_calls_total{method=...}` (counter)
//! - `cirrus_qs_rpc_errors_total{method=...}` (counter)
//! - `cirrus_qs_queue_depth` (gauge — current queue size)
//! - `cirrus_qs_runs_total{exit_status=...}` (counter — incremented
//!   on each engine `RunResult` returned through `queue_item_execute`
//!   / queue worker)
//! - `cirrus_qs_documents_total{name=...}` (counter — bumped by the
...
//! so cirrus-qs without `--features metrics` is unchanged.
...
pub fn rpc_call(method: &str) {
    metrics::counter!("cirrus_qs_rpc_calls_total", "method" => method.to_string()).increment(1);
}

/// Record an RPC error response.
#[allow(dead_code)]
pub fn rpc_error(method: &str) {
    metrics::counter!("cirrus_qs_rpc_errors_total", "method" => method.to_string()).increment(1);
}

/// Update the queue-depth gauge.
#[allow(dead_code)]
pub fn queue_depth(depth: usize) {
    metrics::gauge!("cirrus_qs_queue_depth").set(depth as f64);
}
...
pub fn document(name: &str) {
    metrics::counter!("cirrus_qs_documents_total", "name" => name.to_string()).increment(1);
}
