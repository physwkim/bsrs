...
use bsrs_qs::{Registry, Server, ServerShutdown};
...
async fn shutdown_aborts_running_queue_task() {
    ...
    let mut reg = Registry::new();
    let factory: bsrs_qs::PlanFactory = Arc::new(move |_reg, _args| {
...
impl bsrs_qs::LuaEvaluator for MockEval {
    async fn eval(&self, source: &str) -> bsrs_qs::EvalResult {
        let ms: u64 = source
            .split_whitespace()
            .next()
...
fn spawn_server_with_eval(
    reg: Registry,
    port: u16,
    ev: Arc<dyn bsrs_qs::LuaEvaluator>,
) -> ServerShutdown {
    let ep = endpoint(port);
...
#[async_trait::async_trait]
impl bsrs_qs::LuaEvaluator for PanickingEval {
    async fn eval(&self, _source: &str) -> bsrs_qs::EvalResult {
...
async fn curve_server_enabled_via_builder_key_round_trip() {
    if !bsrs_qs::curve_supported() {
        eprintln!("CURVE not available in this libzmq build — skipping");
        return;
    }
    let (server_pub, server_priv) = bsrs_qs::generate_zmq_keys().expect("generate server keypair");
    let (client_pub, client_priv) = bsrs_qs::generate_zmq_keys().expect("generate client keypair");
