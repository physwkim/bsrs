//! Lua environment for the bsrs REPL. Wraps bsrs types and plan
...
/// Holder for an opaque bsrs device. Wraps the trait-object Arc and
...
impl UserData for LuaDevice {
    fn add_methods<M: UserDataMethods<Self>>(methods: &mut M) {
        ...
        // ---- bluesky-style short-name methods (mirrors bsrs-core::ext) ----
        ...
        methods.add_method("locate", |lua, dev, ()| {
            ...
            let l = bsrs_core::runtime::bsrs_runtime()
                .block_on(lo.locate_dyn())
        ...
        methods.add_method("read", |lua, dev, ()| {
            ...
            let data = bsrs_core::runtime::bsrs_runtime()
                .block_on(r.read_dyn())
        ...
        methods.add_method("describe", |lua, dev, ()| {
            ...
            let dks = bsrs_core::runtime::bsrs_runtime()
        ...
        methods.add_method("set", |_, dev, v: f64| {
            ...
            let status = bsrs_core::runtime::bsrs_runtime().block_on(mv.set_dyn(v));
        ...
        methods.add_method("trigger", |_, dev, ()| {
            ...
            let status = bsrs_core::runtime::bsrs_runtime().block_on(t.trigger_dyn());
        ...
        methods.add_method("kickoff", |_, dev, ()| {
            ...
            let status = bsrs_core::runtime::bsrs_runtime().block_on(f.kickoff_dyn());
        ...
        methods.add_method("complete", |_, dev, ()| {
            let f = dev
                ...
                .clone()
                .ok_or_else(|| mlua::Error::RuntimeError(format!("{} is not flyable", dev.name)))?;
            let status = bsrs_core::runtime::bsrs_runtime().block_on(f.complete_dyn());
...
/// Lua-side `Status` handle. Wraps a `bsrs_core::Status` (which is
...
pub struct LuaStatus {
    inner: bsrs_core::status::Status,
    label: String,
}

impl LuaStatus {
    /// Build from a `Status`.
    pub fn new(status: bsrs_core::status::Status, label: String) -> Self {
        Self {
            inner: status,
            label,
        }
...
impl bsrs_core::msg::NamedObj for LuaPausableCounter {
    fn name(&self) -> &str {
        &self.name
    }
}
...
impl bsrs_core::msg::PausableObj for LuaPausableCounter {
...
impl UserData for LuaRunEngine {
    fn add_methods<M: UserDataMethods<Self>>(methods: &mut M) {
        methods.add_method("run", |_, this, plan: mlua::AnyUserData| {
            ...
            // Drive the plan to completion on bsrs's own runtime. Lua
            // callbacks run from a sync REPL thread (see main.rs), so
            // `block_on` here is safe.
            let raw = bsrs_core::runtime::bsrs_runtime().block_on(re.run_async(plan));
        ...
        methods.add_method("current_run_uid", |_, this, ()| {
            // current_run_uid is async; block on bsrs runtime — safe
            // from Lua REPL thread.
            let uid = bsrs_core::runtime::bsrs_runtime().block_on(this.re.current_run_uid());
            Ok(uid)
        });
        ...
        methods.add_method("unregister_command", |_, this, name: String| {
            if name == BRIDGE_ERROR_CMD {
                return Err(mlua::Error::RuntimeError(format!(
                    "{name:?} is reserved by the bsrs Lua bridge for error \
        ...
        methods.add_method(
            ...
            |_, this, (name, cb): (String, mlua::Function)| {
                if name == BRIDGE_ERROR_CMD {
                    return Err(mlua::Error::RuntimeError(format!(
                        "{name:?} is reserved by the bsrs Lua bridge; \
                         overriding it would silently swallow Lua coroutine errors"
                    )));
                }
                let f = cb;
                let handler: bsrs_engine::CustomCommandHandler =
                    Arc::new(move |payload: &(dyn std::any::Any + Send + Sync)| {
                        ...
                        Box::pin(async move {
                            f.call::<()>(txt).map_err(|e| {
                                bsrs_core::error::BsrsError::Plan(format!(
        ...
        methods.add_method(
            ...
            |lua, this, cb: Option<mlua::Function>| {
                match cb {
                    ...
                    Some(f) => {
                        let lua = lua.clone();
                        let v: bsrs_engine::MdValidator = Arc::new(move |md| {
                            let table = json_md_to_lua_table(&lua, md.clone())?;
                            match f.call::<Option<String>>(table) {
                                Ok(None) => Ok(()),
                                Ok(Some(msg)) => Err(bsrs_core::error::BsrsError::Plan(msg)),
                                Err(e) => Err(bsrs_core::error::BsrsError::Plan(format!(
                                    "Lua md_validator: {e}"
                                ))),
                            }
                        });
        ...
        methods.add_method(
            ...
            |lua, this, cb: Option<mlua::Function>| {
                match cb {
                    ...
                    Some(f) => {
                        let lua = lua.clone();
                        let n: bsrs_engine::MdNormalizer = Arc::new(move |md| {
                            let table = json_md_to_lua_table(&lua, md.clone())?;
        ...
        methods.add_method(
            ...
            |lua, this, cb: Option<mlua::Function>| {
                match cb {
                    ...
                    Some(f) => {
                        let lua = lua.clone();
                        let s: bsrs_engine::ScanIdSource = Arc::new(move |md| {
                            let table = json_md_to_lua_table(&lua, md.clone())?;
        ...
        methods.add_method("set_before_plan", |_, this, cb: Option<mlua::Function>| {
            match cb {
                ...
                Some(f) => {
                    let h: bsrs_engine::PlanHook = Arc::new(move || {
                        let _ = f.call::<()>(());
                    });
        ...
        methods.add_method(
            ...
            |_, this, cb: Option<mlua::Function>| {
                match cb {
                    ...
                    Some(f) => {
                        let h: bsrs_engine::InputHandler = Arc::new(move |prompt| {
                            let f = f.clone();
                            Box::pin(async move {
        ...
        methods.add_method("register_pausable", |_, this, dev: mlua::AnyUserData| {
            ...
            bsrs_core::runtime::bsrs_runtime().block_on(this.re.register_pausable(p));
            Ok(())
        });
        ...
        methods.add_method("unregister_pausable", |_, this, arg: LuaValue| {
            ...
            bsrs_core::runtime::bsrs_runtime().block_on(this.re.unregister_pausable(&name));
            Ok(())
        });
        ...
        methods.add_method(
            ...
            |_, this, (plan, opts): (mlua::AnyUserData, mlua::Table)| {
                ...
                let mut subs: Vec<bsrs_engine::DocumentCallback> = Vec::new();
                match opts.get::<LuaValue>("subs") {
                    Ok(LuaValue::Nil) => {}
                    Ok(LuaValue::Table(t)) => {
                        for v in t.sequence_values::<mlua::Function>().flatten() {
                            let f = v;
                            let dcb: bsrs_engine::DocumentCallback =
                                Arc::new(move |doc: &bsrs_event_model::Document| {
                ...
                let opts = bsrs_engine::RunOptions { md, subs };
                let raw =
                    bsrs_core::runtime::bsrs_runtime().block_on(re.run_async_with(plan, opts));
...
pub fn build_lua(re: Arc<RunEngine>) -> mlua::Result<Lua> {
    ...
    let bridge_handler: bsrs_engine::CustomCommandHandler =
        Arc::new(|payload: &(dyn std::any::Any + Send + Sync)| {
            let msg = payload
                .downcast_ref::<String>()
                .cloned()
                .unwrap_or_else(|| "Lua coroutine error (no detail)".into());
            Box::pin(async move { Err(bsrs_core::error::BsrsError::Plan(msg)) })
        });
...
fn register_plan_factories(lua: &Lua) -> mlua::Result<()> {
    ...
    let f = lua.create_function(|_, (dets, num): (mlua::Table, usize)| {
        ...
        let plan = bsrs_plans::count(detectors, num);
        Ok(LuaPlan {
    ...
    let f = lua.create_function(
        |_, (dets, motor, start, stop, num): (mlua::Table, mlua::AnyUserData, f64, f64, usize)| {
            ...
            let plan = bsrs_plans::scan(detectors, movable, readable, start, stop, num);
            Ok(LuaPlan {
    ...
    let f = lua.create_function(|_, (motor, delta): (mlua::AnyUserData, f64)| {
        ...
        let plan = bsrs_plans::stubs::mvr(loc, delta);
        Ok(LuaPlan {
    ...
    let f = lua.create_function(|_, secs: f64| {
        let plan = bsrs_plans::stubs::sleep(std::time::Duration::from_secs_f64(secs));
        Ok(LuaPlan {
    ...
    let f = lua.create_function(|_, ()| {
        let plan = bsrs_plans::stubs::null();
        Ok(LuaPlan {
            label: "null".into(),
            kind: TMutex::new(Some(LuaPlanKind::Prebuilt(plan))),
        })
...
fn lua_table_to_data_key(t: &mlua::Table) -> mlua::Result<bsrs_event_model::DataKey> {
    ...
    let dtype = match dtype_str.as_str() {
        "number" => bsrs_event_model::Dtype::Number,
        "string" => bsrs_event_model::Dtype::String,
        "boolean" => bsrs_event_model::Dtype::Boolean,
        "integer" => bsrs_event_model::Dtype::Integer,
        "array" => bsrs_event_model::Dtype::Array,
        other => {
            return Err(mlua::Error::RuntimeError(format!(
...
///
/// Lists must match `bsrs-event-model` structs: only fields without
/// `#[serde(default)]` (and that aren't `Option<T>` with a default) go
/// here. Verified against `crates/bsrs-event-model/src/documents.rs`.
...
) -> Result<bsrs_event_model::Document, String> {
    use bsrs_event_model::Document as D;
...
fn document_to_name_body(d: &bsrs_event_model::Document) -> (&'static str, serde_json::Value) {
...
) -> bsrs_core::error::Result<mlua::Table> {
    let table = lua
        .create_table()
        .map_err(|e| bsrs_core::error::BsrsError::Plan(format!("Lua table create: {e}")))?;
    for (k, v) in md {
        let lv = json_to_lua_value(lua, &v)
            .map_err(|e| bsrs_core::error::BsrsError::Plan(format!("Lua convert: {e}")))?;
        table
            .set(k, lv)
            .map_err(|e| bsrs_core::error::BsrsError::Plan(format!("Lua set: {e}")))?;
    }
    Ok(table)
}
...
fn register_msg_namespace(lua: &Lua) -> mlua::Result<()> {
    ...
    //   - return "<err string>" → resolves Err(BsrsError::Plan(s))
    ...
    msg.set(
        ...
        lua.create_function(|_, (factories, timeout_secs): (mlua::Table, Option<f64>)| {
            let mut fs: Vec<
                Arc<
                    dyn Fn() -> futures::future::BoxFuture<'static, bsrs_core::error::Result<()>>
                        + Send
                        + Sync,
                >,
            > = Vec::new();
            for v in factories.sequence_values::<LuaValue>().flatten() {
                if let LuaValue::Function(f) = v {
                    ...
                    let f_arc: Arc<
                        dyn Fn()
                                -> futures::future::BoxFuture<'static, bsrs_core::error::Result<()>>
                            + Send
                            + Sync,
                    > = Arc::new(move || {
                        ...
                        Box::pin(async move {
                            match res {
                                Ok(None) => Ok(()),
                                Ok(Some(err)) => Err(bsrs_core::error::BsrsError::Plan(err)),
                                Err(e) => Err(bsrs_core::error::BsrsError::Plan(format!(
                                    "wait_for factory: {e}"
                                ))),
                            }
    ...
    msg.set(
        ...
        lua.create_function(|_, (cb, name): (mlua::Function, Option<String>)| {
            ...
            Ok(LuaMsg(Msg::Subscribe {
                cb: cb_arc,
                filter: bsrs_event_model::DocFilter::All,
            }))
        })?,
    )?;
    // unsubscribe(id)
...
fn msg_result_to_lua(lua: &Lua, r: bsrs_engine::MsgResult) -> LuaValue {
    use bsrs_engine::MsgResult;
    match r {
...
fn lua_table_to_metadata(t: Option<mlua::Table>) -> mlua::Result<bsrs_core::msg::RunMetadata> {
    let mut m = bsrs_core::msg::RunMetadata::default();
...
fn register_bp(lua: &Lua) -> mlua::Result<()> {
        ...
        lua.create_function(
            |_, (dets_t, trigs_t, num): (mlua::Table, mlua::Table, usize)| {
                ...
                Ok(wrap_prebuilt(
                    ...
                    bsrs_plans::count_with_trigger(dets, trigs, num),
                ))
            },
        )?,
    )?;
    bp.set(
        ...
        lua.create_function(
            |_, (dt, mu, start, stop, num): (mlua::Table, mlua::AnyUserData, f64, f64, usize)| {
                ...
                Ok(wrap_prebuilt(
                    format!("scan(n={num})"),
                    bsrs_plans::scan(dets(&dt)?, mv, rd, start, stop, num),
                ))
            },
        )?,
    )?;
    bp.set(
        ...
        lua.create_function(
            |_, (dt, mu, points_t): (mlua::Table, mlua::AnyUserData, mlua::Table)| {
                ...
                Ok(wrap_prebuilt(
                    "list_scan",
                    bsrs_plans::list_scan(dets(&dt)?, mv, rd, vec_f64(&points_t)?),
                ))
            },
        )?,
    )?;
    bp.set(
        ...
        lua.create_function(
            |_, (dt, mu, start, stop, num): (mlua::Table, mlua::AnyUserData, f64, f64, usize)| {
                ...
                let current = bsrs_core::runtime::bsrs_runtime()
                    .block_on(lo.locate_dyn())
                    .map(|l| l.readback)
                    .unwrap_or(0.0);
                Ok(wrap_prebuilt(
                    format!("rel_scan(n={num})"),
                    bsrs_plans::rel_scan(dets(&dt)?, mv, rd, current, start, stop, num),
                ))
            },
        )?,
    )?;
    bp.set(
        ...
        lua.create_function(
            |_, (dt, mu, points_t): (mlua::Table, mlua::AnyUserData, mlua::Table)| {
                ...
                Ok(wrap_prebuilt(
                    "rel_list_scan",
                    bsrs_plans::rel_list_scan(dets(&dt)?, lo, rd, vec_f64(&points_t)?),
                ))
            },
        )?,
    )?;
    ...
    // grid_scan: 2D only in bsrs-plans. Pass {axes={a1, a2}} where each
    ...
    bp.set(
        ...
        lua.create_function(|_, (dt, axes_t): (mlua::Table, mlua::Table)| {
            ...
            Ok(wrap_prebuilt(
                "list_grid_scan",
                bsrs_plans::list_grid_scan(dets, axes),
            ))
        })?,
    )?;
    bp.set(
        ...
        lua.create_function(|_, (dt, num, axes_t): (mlua::Table, usize, mlua::Table)| {
            ...
            Ok(wrap_prebuilt(
                ...
                bsrs_plans::inner_product_scan(dets, num, axes),
            ))
        })?,
    )?;
    bp.set(
        ...
        lua.create_function(
            |_, (dt, axes_t, points_t): (mlua::Table, mlua::Table, mlua::Table)| {
                ...
                Ok(wrap_prebuilt(
                    ...
                    bsrs_plans::scan_nd(dets, axes, points),
                ))
            },
        )?,
    )?;
        ...
        lua.create_function(
            |_, (dt, mu, start, stop, num): (mlua::Table, mlua::AnyUserData, f64, f64, usize)| {
                ...
                Ok(wrap_prebuilt(
                    ...
                    bsrs_plans::log_scan(dets(&dt)?, mv, rd, start, stop, num),
                ))
            },
        )?,
    )?;
...
fn axes_table_to_list_grid_axes(t: &mlua::Table) -> mlua::Result<Vec<bsrs_plans::ListGridAxis>> {
    let mut out = Vec::new();
    for v in t.clone().sequence_values::<mlua::Table>() {
        let row = v?;
        let mu: mlua::AnyUserData = row.get("motor")?;
...
fn register_bps(lua: &Lua) -> mlua::Result<()> {
    ...
    bps.set(
        ...
        lua.create_function(|_, (n, f): (usize, mlua::Function)| {
            ...
            Ok(wrap_prebuilt(
                format!("repeater(n={n})"),
                bsrs_plans::preprocessors::pchain(plans),
            ))
        })?,
    )?;
...
fn register_bpp(lua: &Lua) -> mlua::Result<()> {
    ...
    use bsrs_plans::preprocessors as pp;
...
/// Block on a bsrs async op from a Lua callback, formatting errors
...
where
    F: std::future::Future<Output = bsrs_core::error::Result<T>>,
{
    bsrs_core::runtime::bsrs_runtime()
        .block_on(fut)
...
mod ad_lua_smoke {
    ...
    //! factories call `bsrs_runtime().block_on(...)` synchronously
