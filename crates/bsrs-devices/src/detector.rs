...
/// bsrs-devices.
pub use bsrs_protocols_async::{DetectorTrigger, TriggerInfo};
    ...
    W: DetectorWriter,
{
    ...
    /// Errors with `BsrsError::State` if the detector has not been staged
    ...
    async fn trigger(&self) -> Status {
        ...
        match arm.await {
            Ok(()) => match self.control.wait_for_idle().await {
                Ok(()) => Status::done(),
                Err(e) => Status::fail(bsrs_core::status::StatusError::Failed(format!(
                    "wait_for_idle: {e}"
                ))),
            },
            Err(e) => Status::fail(e),
    ...
    W: DetectorWriter,
{
    ...
    async fn kickoff(&self) -> Status {
        ...
        if let Err(e) = self.control.prepare(info.clone()).await {
            return Status::fail(bsrs_core::status::StatusError::Failed(format!(
                "prepare: {e}"
            )));
        }
    ...
    W: DetectorWriter,
{
    ...
    async fn prepare(&self, info: TriggerInfo) -> bsrs_core::status::Status {
    ...
    W: DetectorWriter + Send + Sync + 'static,
{
    ...
    async fn collect_stream_docs_dyn(
        &self,
        descriptor: &str,
    ) -> Result<Vec<bsrs_event_model::Document>> {
        let up_to = WritesStreamAssets::get_index(self).await?;
        let docs = WritesStreamAssets::collect_asset_docs(self, up_to, descriptor)
            .map(|asset| match asset {
                StreamAsset::Resource(r) => bsrs_event_model::Document::StreamResource(r),
                StreamAsset::Datum(d) => bsrs_event_model::Document::StreamDatum(d),
            })
            .collect::<Vec<_>>()
            .await;
        Ok(docs)
...
mod tests {
    use super::*;
    use bsrs_protocols_async::Preparable;
    use futures::stream;
    use std::sync::Arc;
