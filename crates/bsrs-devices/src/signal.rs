...
use bsrs_core::status::{Status, StatusError, SubToken};
use bsrs_core::Kind;
use bsrs_event_model::DataKey;
use bsrs_protocols_async::{
...
pub use bsrs_core::Kind as SignalKind;
    ...
    A: Access,
{
    ...
    /// reports in [`DataKey.source`](bsrs_core::DataKey). Sync (no backend
...
#[async_trait]
impl<T, B, A> bsrs_core::msg::NamedObj for Signal<T, B, A>
where
    T: Clone + Send + Sync + Serialize + 'static,
    B: SignalBackend<T>,
    A: Access,
...
#[async_trait]
impl<T, B, A> bsrs_core::msg::ReadableObj for Signal<T, B, A>
where
    T: Clone + Send + Sync + Serialize + 'static,
    B: SignalBackend<T>,
    A: Readable,
...
#[async_trait]
impl<T, B, A> bsrs_protocols_async::Stageable for Signal<T, B, A>
where
    T: Clone + Send + Sync + Serialize + 'static,
    B: SignalBackend<T>,
    A: Readable,
...
#[async_trait]
impl<T, B, A> bsrs_core::msg::StageableObj for Signal<T, B, A>
where
    T: Clone + Send + Sync + Serialize + 'static,
    B: SignalBackend<T>,
    A: Readable,
...
mod tests {
    ...
    async fn access_roles_expose_the_right_surface() {
        ...
        let backend = || {
            Arc::new(bsrs_backend_soft::SoftSignalBackend::new(
                0.0_f64,
                Dtype::Number,
            ))
        };
    ...
    async fn signal_x_trigger_writes_backend_default() {
        ...
        let backend = Arc::new(bsrs_backend_soft::SoftSignalBackend::new(
            42.0_f64,
            Dtype::Number,
        ));
