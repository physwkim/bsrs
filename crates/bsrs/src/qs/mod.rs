//! bsrs-qs — queueserver-compatible 0MQ JSON-RPC daemon.
//!
...
//! drives [`crate::engine::RunEngine`] for plan execution.
//!
...
//! - Document broadcast via bsrs-callbacks `ZmqDocumentSink` (separate
//!   PUB socket).
...
//! ## RBAC (opt-in)
//!
//! Without a `permissions.toml`, bsrs-qs runs *permissive* — every
//! method is allowed for every caller. Pass
//! [`ServerBuilder::permissions_path`] (or `bsrs qs-manager
...
//! bluesky-queueserver-specific and don't translate to bsrs's
...
//!
//! ```ignore
//! use crate::qs::{Server, Registry};
//! use crate::backends::soft::SoftDetector;
//! use std::sync::Arc;
//!
//! # async fn run() -> crate::core::error::Result<()> {
//! let det = SoftDetector::new("det1");
//! let mut reg = Registry::new();
//! reg.register_readable("det1", det as Arc<dyn crate::core::msg::ReadableObj>);
