...
async fn run_wired(args: FrameSourceArgs) -> i32 {
    use bsrs::callbacks::{Serializer, ZmqDocumentSink};
    use bsrs::event_model::Document;
    use bsrs::protocols_async::{DetectorWriter, FrameSink, FrameSource, StreamAsset};
    use bsrs::stream::sinks::Hdf5FrameSink;
    use bsrs::stream::sources::PvaMonitorSource;
    use bsrs::stream::FramePipe;
    ...
    let doc_loop = tokio::spawn(async move {
        ...
        loop {
            ...
            while let Some(asset) = stream.next().await {
                ...
                if let Err(e) = bsrs::engine::DocumentSink::dispatch(&*doc_sink_clone, &doc).await {
