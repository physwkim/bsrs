...
impl UserData for LuaDevice {
    fn add_methods<M: UserDataMethods<Self>>(methods: &mut M) {
        ...
        methods.add_method("locate", |lua, dev, ()| {
            ...
            let l = crate::core::runtime::bsrs_runtime()
                .block_on(lo.locate_dyn())
        ...
        methods.add_method("read", |lua, dev, ()| {
            ...
            let data = crate::core::runtime::bsrs_runtime()
                .block_on(r.read_dyn())
        ...
        methods.add_method("describe", |lua, dev, ()| {
            ...
            let dks = crate::core::runtime::bsrs_runtime()
        ...
        methods.add_method("set", |_, dev, v: f64| {
            ...
            let status = crate::core::runtime::bsrs_runtime().block_on(mv.set_dyn(v));
        ...
        methods.add_method("trigger", |_, dev, ()| {
            ...
            let status = crate::core::runtime::bsrs_runtime().block_on(t.trigger_dyn());
        ...
        methods.add_method("kickoff", |_, dev, ()| {
            ...
            let status = crate::core::runtime::bsrs_runtime().block_on(f.kickoff_dyn());
        ...
        methods.add_method("complete", |_, dev, ()| {
            let f = dev
                ...
                .clone()
                .ok_or_else(|| mlua::Error::RuntimeError(format!("{} is not flyable", dev.name)))?;
            let status = crate::core::runtime::bsrs_runtime().block_on(f.complete_dyn());
...
/// Lua-side `Status` handle. Wraps a `crate::core::Status` (which is
...
pub struct LuaStatus {
    inner: crate::core::status::Status,
    label: String,
}

impl LuaStatus {
    /// Build from a `Status`.
    pub fn new(status: crate::core::status::Status, label: String) -> Self {
        Self {
            inner: status,
            label,
        }
...
impl crate::core::msg::NamedObj for LuaPausableCounter {
    fn name(&self) -> &str {
        &self.name
    }
}
...
impl crate::core::msg::PausableObj for LuaPausableCounter {
    async fn pause_dyn(&self) -> Result<(), crate::core::error::BsrsError> {
        self.paused
            .fetch_add(1, std::sync::atomic::Ordering::SeqCst);
        Ok(())
    }
    async fn resume_dyn(&self) -> Result<(), crate::core::error::BsrsError> {
        self.resumed
            .fetch_add(1, std::sync::atomic::Ordering::SeqCst);
        Ok(())
    }
...
impl UserData for LuaRunEngine {
    fn add_methods<M: UserDataMethods<Self>>(methods: &mut M) {
        methods.add_method("run", |_, this, plan: mlua::AnyUserData| {
            ...
            let raw = crate::core::runtime::bsrs_runtime().block_on(re.run_async(plan));
        ...
        methods.add_method("current_run_uid", |_, this, ()| {
            ...
            let uid = crate::core::runtime::bsrs_runtime().block_on(this.re.current_run_uid());
            Ok(uid)
        });
        ...
        methods.add_method(
            ...
            |_, this, (name, cb): (String, mlua::Function)| {
                ...
                let f = cb;
                let handler: crate::engine::CustomCommandHandler =
        ...
        methods.add_method(
            ...
            |lua, this, cb: Option<mlua::Function>| {
                match cb {
                    ...
                    Some(f) => {
                        let lua = lua.clone();
                        let v: crate::engine::MdValidator = Arc::new(move |md| {
                            let table = json_md_to_lua_table(&lua, md.clone())?;
                            match f.call::<Option<String>>(table) {
                                Ok(None) => Ok(()),
                                Ok(Some(msg)) => Err(crate::core::error::BsrsError::Plan(msg)),
                                Err(e) => Err(crate::core::error::BsrsError::Plan(format!(
                                    "Lua md_validator: {e}"
                                ))),
                            }
                        });
        ...
        methods.add_method(
            ...
            |lua, this, cb: Option<mlua::Function>| {
                match cb {
                    ...
                    Some(f) => {
                        let lua = lua.clone();
                        let s: crate::engine::ScanIdSource = Arc::new(move |md| {
                            let table = json_md_to_lua_table(&lua, md.clone())?;
        ...
        methods.add_method("set_before_plan", |_, this, cb: Option<mlua::Function>| {
            match cb {
                ...
                Some(f) => {
                    let h: crate::engine::PlanHook = Arc::new(move || {
                        let _ = f.call::<()>(());
                    });
        ...
        methods.add_method(
            ...
            |_, this, cb: Option<mlua::Function>| {
                match cb {
                    ...
                    Some(f) => {
                        let h: crate::engine::InputHandler = Arc::new(move |prompt| {
                            let f = f.clone();
                            Box::pin(async move {
        ...
        methods.add_method("register_pausable", |_, this, dev: mlua::AnyUserData| {
            ...
            crate::core::runtime::bsrs_runtime().block_on(this.re.register_pausable(p));
            Ok(())
        });
        ...
        methods.add_method("unregister_pausable", |_, this, arg: LuaValue| {
            ...
            crate::core::runtime::bsrs_runtime().block_on(this.re.unregister_pausable(&name));
            Ok(())
        });
        ...
        methods.add_method(
            ...
            |_, this, (plan, opts): (mlua::AnyUserData, mlua::Table)| {
                ...
                let mut subs: Vec<crate::engine::DocumentCallback> = Vec::new();
                match opts.get::<LuaValue>("subs") {
                    Ok(LuaValue::Nil) => {}
                    Ok(LuaValue::Table(t)) => {
                        for v in t.sequence_values::<mlua::Function>().flatten() {
                            let f = v;
                            let dcb: crate::engine::DocumentCallback =
                ...
                let opts = crate::engine::RunOptions { md, subs };
                let raw =
                    crate::core::runtime::bsrs_runtime().block_on(re.run_async_with(plan, opts));
...
pub fn build_lua(re: Arc<RunEngine>) -> mlua::Result<Lua> {
    ...
    let bridge_handler: crate::engine::CustomCommandHandler =
        Arc::new(|payload: &(dyn std::any::Any + Send + Sync)| {
            let msg = payload
                .downcast_ref::<String>()
                .cloned()
                .unwrap_or_else(|| "Lua coroutine error (no detail)".into());
            Box::pin(async move { Err(crate::core::error::BsrsError::Plan(msg)) })
        });
    ...
    #[cfg(feature = "pva")]
    {
        use crate::host::pva_devices::{PvaDetector, PvaMotor};
        let f = lua.create_function(|_, (name, val_pv, rbv_pv): (String, String, String)| {
    ...
    #[cfg(feature = "tiled")]
    crate::host::lua_tiled::register(&lua)?;
...
fn register_plan_factories(lua: &Lua) -> mlua::Result<()> {
    ...
    let f = lua.create_function(|_, (dets, num): (mlua::Table, usize)| {
        ...
        let plan = crate::plans::count(detectors, num);
        Ok(LuaPlan {
    ...
    let f = lua.create_function(|_, (motor, delta): (mlua::AnyUserData, f64)| {
        ...
        let plan = crate::plans::stubs::mvr(loc, delta);
        Ok(LuaPlan {
    ...
    let f = lua.create_function(|_, secs: f64| {
        let plan = crate::plans::stubs::sleep(std::time::Duration::from_secs_f64(secs));
        Ok(LuaPlan {
    ...
    let f = lua.create_function(|_, ()| {
        let plan = crate::plans::stubs::null();
        Ok(LuaPlan {
            label: "null".into(),
            kind: TMutex::new(Some(LuaPlanKind::Prebuilt(plan))),
        })
...
) -> Result<crate::event_model::Document, String> {
    use crate::event_model::Document as D;
...
fn document_to_name_body(d: &crate::event_model::Document) -> (&'static str, serde_json::Value) {
...
) -> crate::core::error::Result<mlua::Table> {
    let table = lua
        .create_table()
        .map_err(|e| crate::core::error::BsrsError::Plan(format!("Lua table create: {e}")))?;
    for (k, v) in md {
        let lv = json_to_lua_value(lua, &v)
            .map_err(|e| crate::core::error::BsrsError::Plan(format!("Lua convert: {e}")))?;
        table
            .set(k, lv)
            .map_err(|e| crate::core::error::BsrsError::Plan(format!("Lua set: {e}")))?;
    }
    Ok(table)
}
...
fn register_msg_namespace(lua: &Lua) -> mlua::Result<()> {
    ...
    msg.set(
        ...
        lua.create_function(|_, (factories, timeout_secs): (mlua::Table, Option<f64>)| {
            let mut fs: Vec<
                Arc<
                    dyn Fn() -> futures::future::BoxFuture<'static, crate::core::error::Result<()>>
                        + Send
                        + Sync,
                >,
            > = Vec::new();
                ...
                if let LuaValue::Function(f) = v {
                    ...
                    > = Arc::new(move || {
                        ...
                        Box::pin(async move {
                            match res {
                                Ok(None) => Ok(()),
                                Ok(Some(err)) => Err(crate::core::error::BsrsError::Plan(err)),
                                Err(e) => Err(crate::core::error::BsrsError::Plan(format!(
                                    "wait_for factory: {e}"
                                ))),
                            }
    ...
    msg.set(
        ...
        lua.create_function(|_, (cb, name): (mlua::Function, Option<String>)| {
            ...
            Ok(LuaMsg(Msg::Subscribe {
                cb: cb_arc,
                filter: crate::event_model::DocFilter::All,
            }))
        })?,
    )?;
    // unsubscribe(id)
...
fn msg_result_to_lua(lua: &Lua, r: crate::engine::MsgResult) -> LuaValue {
    use crate::engine::MsgResult;
    match r {
...
fn lua_table_to_metadata(t: Option<mlua::Table>) -> mlua::Result<crate::core::msg::RunMetadata> {
    let mut m = crate::core::msg::RunMetadata::default();
        ...
        lua.create_function(
            |_, (dets_t, trigs_t, num): (mlua::Table, mlua::Table, usize)| {
                ...
                Ok(wrap_prebuilt(
                    ...
                    crate::plans::count_with_trigger(dets, trigs, num),
                ))
            },
        )?,
    )?;
    ...
    bp.set(
        ...
        lua.create_function(
            |_, (dt, mu, points_t): (mlua::Table, mlua::AnyUserData, mlua::Table)| {
                ...
                Ok(wrap_prebuilt(
                    "list_scan",
                    crate::plans::list_scan(dets(&dt)?, mv, rd, vec_f64(&points_t)?),
                ))
            },
        )?,
    )?;
    bp.set(
        ...
        lua.create_function(
            |_, (dt, mu, start, stop, num): (mlua::Table, mlua::AnyUserData, f64, f64, usize)| {
                ...
                let current = crate::core::runtime::bsrs_runtime()
                    .block_on(lo.locate_dyn())
                    .map(|l| l.readback)
                    .unwrap_or(0.0);
                Ok(wrap_prebuilt(
                    format!("rel_scan(n={num})"),
                    crate::plans::rel_scan(dets(&dt)?, mv, rd, current, start, stop, num),
                ))
            },
        )?,
    )?;
    bp.set(
        ...
        lua.create_function(
            |_, (dt, mu, points_t): (mlua::Table, mlua::AnyUserData, mlua::Table)| {
                ...
                Ok(wrap_prebuilt(
                    "rel_list_scan",
                    crate::plans::rel_list_scan(dets(&dt)?, lo, rd, vec_f64(&points_t)?),
                ))
            },
        )?,
    )?;
    ...
    bp.set(
        ...
        lua.create_function(|_, (dt, num, axes_t): (mlua::Table, usize, mlua::Table)| {
            ...
            Ok(wrap_prebuilt(
                ...
                crate::plans::inner_product_scan(dets, num, axes),
            ))
        })?,
    )?;
    bp.set(
        ...
        lua.create_function(
            |_, (dt, axes_t, points_t): (mlua::Table, mlua::Table, mlua::Table)| {
                ...
                Ok(wrap_prebuilt(
                    ...
                    crate::plans::scan_nd(dets, axes, points),
                ))
            },
        )?,
    )?;
        ...
        lua.create_function(
            |_, (dt, mu, start, stop, num): (mlua::Table, mlua::AnyUserData, f64, f64, usize)| {
                ...
                Ok(wrap_prebuilt(
                    ...
                    crate::plans::log_scan(dets(&dt)?, mv, rd, start, stop, num),
                ))
            },
        )?,
    )?;
...
fn axes_table_to_list_grid_axes(t: &mlua::Table) -> mlua::Result<Vec<crate::plans::ListGridAxis>> {
    let mut out = Vec::new();
    for v in t.clone().sequence_values::<mlua::Table>() {
        let row = v?;
        let mu: mlua::AnyUserData = row.get("motor")?;
...
fn register_bps(lua: &Lua) -> mlua::Result<()> {
    ...
    bps.set(
        ...
        lua.create_function(|_, (n, f): (usize, mlua::Function)| {
            ...
            Ok(wrap_prebuilt(
                format!("repeater(n={n})"),
                crate::plans::preprocessors::pchain(plans),
            ))
        })?,
    )?;
...
fn register_bpp(lua: &Lua) -> mlua::Result<()> {
    ...
    use crate::plans::preprocessors as pp;
...
where
    F: std::future::Future<Output = crate::core::error::Result<T>>,
{
    crate::core::runtime::bsrs_runtime()
        .block_on(fut)
...
#[cfg(feature = "ca")]
#[derive(Clone)]
pub struct LuaAdCam(Arc<crate::host::areadetector::AreaDetectorCam>);
...
#[cfg(feature = "ca")]
#[derive(Clone)]
pub struct LuaAdFile(Arc<crate::host::areadetector::NdFile>);
...
#[cfg(feature = "ca")]
#[derive(Clone)]
pub struct LuaAdStats(Arc<crate::host::areadetector::NdStats>);
...
#[cfg(feature = "ca")]
#[derive(Clone)]
pub struct LuaAdRoi(Arc<crate::host::areadetector::NdRoi>);
...
fn install_areadetector_factories(lua: &Lua) -> mlua::Result<()> {
    ...
    let f = lua.create_function(
        |_, (file_ud, source_port, siblings_tbl): (mlua::AnyUserData, String, mlua::Table)| {
            ...
            drop(file_ref);
            let mut siblings: Vec<Arc<crate::host::areadetector::NdFile>> = Vec::new();
