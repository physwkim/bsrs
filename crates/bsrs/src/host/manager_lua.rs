...
//! engine, then implements [`crate::qs::LuaEvaluator`] so the
...
//!   Goes through `bsrs qs re abort` from another terminal. The
//!   engine's abort flag propagates through the `bsrs_runtime`
...
use crate::host::lua_env::{build_lua, LuaDevice};
...
pub struct ManagerLuaState {
    ...
    /// often calls back into `bsrs_runtime().block_on(...)`,
...
impl LuaEvaluator for ManagerLuaState {
    async fn eval(&self, source: &str) -> EvalResult {
        ...
        // call `bsrs_runtime().block_on(...)` (RE:run, motor:set,
...
mod tests {
    ...
    impl crate::core::msg::NamedObj for Diffractometer {
        fn name(&self) -> &str {
            &self.name
        }
    }
    ...
    async fn lua_methods_proc_macro_round_trip() {
        ...
        struct StubReadable;
        impl crate::core::msg::NamedObj for StubReadable {
            fn name(&self) -> &str {
                "dx"
            }
        }
        #[async_trait::async_trait]
        impl crate::core::msg::ReadableObj for StubReadable {
            async fn read_dyn(
                &self,
        ...
        reg.register_readable(
            "dx",
            Arc::new(StubReadable) as Arc<dyn crate::core::msg::ReadableObj>,
        );
