//! Bluesky-style short-name extensions for the bsrs role traits.
//!
//! The role traits in [`crate::core::msg`] use `_dyn` suffixes (`read_dyn`,
...
//! BOTH traits are imported in the same scope; the bsrs prelude
...
pub trait MovableExt: MovableObj {
    ...
    async fn move_to(&self, value: f64) -> Result<()> {
        ...
        s.await
            .map_err(|e| crate::core::error::BsrsError::Backend(format!("move failed: {e:?}")))
    }
}
