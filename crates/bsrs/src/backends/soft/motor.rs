...
impl SoftMotor {
    ...
    pub async fn read(&self) -> Result<HashMap<String, ReadingValue>> {
        let r = self
            ...
            .get_reading()
            .await
            .map_err(|_| crate::core::error::BsrsError::Backend("soft read".into()))?;
...
impl Locatable<f64> for SoftMotor {
    async fn locate(&self) -> Result<Location<f64>> {
        Ok(Location {
            setpoint: crate::protocols_async::SignalBackend::get_setpoint(self.backend.as_ref())
                .await?,
            readback: crate::protocols_async::SignalBackend::get_value(self.backend.as_ref())
                .await?,
        })
    }
}
