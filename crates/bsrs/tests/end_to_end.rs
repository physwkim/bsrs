//! End-to-end acceptance tests for bsrs.
...
async fn count_plan_emits_expected_document_sequence() {
    ...
    let plan = bsrs::ophyd_async::count(vec![det.clone()], 5);
...
async fn binary_frame_sink_writes_and_emits_stream_docs() {
    ...
    use bsrs::ophyd_async::StreamAsset;
    ...
    impl TempDir {
        pub fn new() -> std::io::Result<Self> {
            ...
            let nanos = SystemTime::now()
                ...
                .unwrap()
                .as_nanos();
            let p = std::env::temp_dir().join(format!("bsrs-test-{nanos}"));
...
async fn rewind_rolls_back_seq_num_so_replayed_save_reemits_same_seq_num() {
    ...
    // (bundlers.py:520-528, 651-656; run_engine.py:2461-2467). bsrs snapshots
...
async fn abort_threads_caller_reason_into_stop_document() {
    ...
    // surfaces it on the RunStop document (:1792). bsrs previously dropped the
...
async fn halt_emits_schema_valid_abort_in_stop_document() {
    ...
    // success|abort|fail. bsrs reports `halt` on `RunResult.exit_status`
    // (its own richer status, consumed by bsrs-qs) but previously emitted
...
async fn sync_facade_runs_blocking_count() {
    ...
    let join = thread::spawn(move || {
        let plan = bsrs::ophyd::count(vec![det_clone], 3);
...
async fn close_run_without_open_run_is_rejected() {
    ...
    // IllegalMessageSequence (run_engine.py:1902-1905); bsrs must fail the
...
async fn kickoff_without_open_run_is_rejected_before_flyer_starts() {
    ...
    // `obj.kickoff()`. bsrs must do the same: the flyer's data lands in the
...
async fn collect_without_open_run_does_not_describe_the_flyer() {
    ...
    // current_run.collect() runs describe_collect. bsrs called
...
async fn dropped_bundle_reads_do_not_leak_into_next_descriptor() {
    ...
    // reach the next descriptor. bsrs previously accumulated descriptor data
