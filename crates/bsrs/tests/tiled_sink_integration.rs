//! Integration test for `TiledSink` (tiled-rs backed).
//!
...
//!
//! This proves the tiled-rs integration is end-to-end functional
...
use bsrs::callbacks::TiledSink;
use bsrs::engine::DocumentSink;
use bsrs::event_model::{Document, ExitStatus, RunStart, RunStop};
...
async fn drop_event_documents_emit_no_http_traffic() {
    ...
    let descriptor = bsrs::event_model::EventDescriptor {
        uid: "d-1".into(),
        run_start: "run-1".into(),
        time: 1.5,
