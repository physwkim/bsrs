//! End-to-end test: REQ → bsrs-qs REP → engine → response.
//!
...
use bsrs::qs::{Registry, Server, ServerShutdown};
...
fn spawn_server(reg: Registry) -> ServerShutdown {
    spawn_server_inner(reg, None)
}

fn spawn_server_with_perms(reg: Registry, perms_path: std::path::PathBuf) -> ServerShutdown {
    spawn_server_inner(reg, Some(perms_path))
}

fn spawn_server_inner(reg: Registry, perms_path: Option<std::path::PathBuf>) -> ServerShutdown {
    // Bind to a wildcard loopback port; the OS assigns a free port (portable
    // to Windows, unlike unix-domain `ipc://`). The resolved control endpoint
    // is read back from the `ServerShutdown` handle for clients to connect to.
...
async fn config_get_returns_implementation_metadata() {
    ...
    assert_eq!(r["config"]["implementation"], "bsrs-qs");
...
async fn manager_stop_shuts_down_server() {
    ...
    probe.connect(shutdown.control_endpoint()).unwrap();
    probe
        .send(
...
async fn function_execute_returns_task_uid_and_item() {
    let reg = Registry::new();
    let shutdown = spawn_server_with_eval(reg, Arc::new(MockEval));
    tokio::time::sleep(Duration::from_millis(300)).await;
    let req = req_socket(shutdown.control_endpoint());
...
async fn plans_allowed_returns_rich_dict() {
    ...
    assert_eq!(entry["module"], "bsrs_qs");
...
impl bsrs::qs::LuaEvaluator for MockEval {
    async fn eval(&self, source: &str) -> bsrs::qs::EvalResult {
        let ms: u64 = source
            .split_whitespace()
            .next()
...
fn spawn_server_with_eval(reg: Registry, ev: Arc<dyn bsrs::qs::LuaEvaluator>) -> ServerShutdown {
...
#[async_trait::async_trait]
impl bsrs::qs::LuaEvaluator for PanickingEval {
    async fn eval(&self, _source: &str) -> bsrs::qs::EvalResult {
...
async fn lua_eval_panic_surfaces_as_failed_task() {
    let reg = Registry::new();
    let shutdown = spawn_server_with_eval(reg, Arc::new(PanickingEval));
    tokio::time::sleep(Duration::from_millis(300)).await;
    let req = req_socket(shutdown.control_endpoint());
...
async fn msgpack_request_receives_msgpack_response() {
    ...
    sock.connect(shutdown.control_endpoint()).unwrap();
    ...
    let json_sock = req_socket(shutdown.control_endpoint());
...
fn curve_req_socket(
    ...
    sock.set_curve_secretkey(client_priv_z85.as_bytes())
        .unwrap();
    sock.connect(control).unwrap();
    sock
}

#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn curve_server_enabled_via_builder_key_round_trip() {
    if !bsrs::qs::curve_supported() {
        eprintln!("CURVE not available in this libzmq build — skipping");
        return;
    }
    let (server_pub, server_priv) = bsrs::qs::generate_zmq_keys().expect("generate server keypair");
    let (client_pub, client_priv) = bsrs::qs::generate_zmq_keys().expect("generate client keypair");
    ...
    let sock = curve_req_socket(
        shutdown.control_endpoint(),
        &server_pub,
        &client_pub,
        &client_priv,
    );
...
async fn no_curve_when_env_var_unset_plaintext_works() {
    ...
    let sock = req_socket(shutdown.control_endpoint());
    let r = rpc(&sock, "ping", json!({}));
    assert_eq!(
        r["success"], true,
