...
impl JsonlSink {
    ...
    pub async fn open(path: impl AsRef<std::path::Path>) -> Result<Self> {
        let f = tokio::fs::OpenOptions::new()
            ...
            .open(path)
            .await
            .map_err(|e| bsrs_core::error::BsrsError::Backend(format!("jsonl open: {e}")))?;
        Ok(Self {
            file: Mutex::new(f),
        })
    }
...
impl DocumentSink for JsonlSink {
    async fn dispatch(&self, doc: &Document) -> Result<()> {
        ...
        let mut line = serde_json::to_vec(doc)
            .map_err(|e| bsrs_core::error::BsrsError::Backend(format!("jsonl encode: {e}")))?;
        line.push(b'\n');
        let mut f = self.file.lock().await;
        f.write_all(&line)
            .await
            .map_err(|e| bsrs_core::error::BsrsError::Backend(format!("jsonl write: {e}")))?;
        ...
        f.flush()
            .await
            .map_err(|e| bsrs_core::error::BsrsError::Backend(format!("jsonl flush: {e}")))?;
        Ok(())
    }
}
...
impl DocumentSink for StderrTraceSink {
    async fn dispatch(&self, doc: &Document) -> Result<()> {
        eprintln!("[bsrs] {}", crate::doc_name::document_name(doc));
        Ok(())
    }
}
...
mod tests {
    use super::*;
    use bsrs_event_model::{Document, ExitStatus, RunStop};
