//! `bsrs_native` — Python bindings to bsrs's RunEngine, soft
//! devices, and a curated set of plan factories. Built as a Python
//! extension module via PyO3.
//!
//! ## Status
//!
//! M7 milestone — the goal is "use bsrs from Python without
//! Python on the IOC host". This first cut covers:
//!
//! - `bsrs_native.SoftMotor(name, initial=0.0)`
//! - `bsrs_native.SoftDetector(name)`
//! - `bsrs_native.RunEngine()` with `.run(plan)` returning a
//!   `(exit_status, run_uid)` tuple.
//! - `bsrs_native.count(detectors, num)`,
//!   `bsrs_native.scan(detectors, motor, motor_reader, start, stop, num)`,
//!   `bsrs_native.grid_scan(…)`, `bsrs_native.rel_scan(…)`,
//!   `bsrs_native.mv(motor, value)` — plan factories.
...
/// `bsrs_native.SoftMotor` — wrapper around `bsrs-backend-soft::SoftMotor`.
#[pyclass(name = "SoftMotor", module = "bsrs_native")]
struct PySoftMotor {
    inner: Arc<SoftMotor>,
}
...
/// `bsrs_native.SoftDetector`.
#[pyclass(name = "SoftDetector", module = "bsrs_native")]
...
#[pyclass(name = "Plan", module = "bsrs_native", unsendable)]
struct PyPlan {
...
/// `bsrs_native.RunEngine` — drives plans synchronously.
#[pyclass(name = "RunEngine", module = "bsrs_native")]
struct PyRunEngine {
    inner: Arc<RunEngine>,
}
...
/// Run a device's `read_dyn` on the bsrs runtime with the GIL released, then
...
/// `bsrs_native.count(detectors, num)` — bluesky `bp.count` mirror.
#[pyfunction]
...
/// `bsrs_native.grid_scan(detectors, m1, m1_reader, s1, e1, n1, m2, m2_reader, s2, e2, n2)`
...
/// `bsrs_native.mv(motor, value)` — bluesky `bps.mv` mirror: set the motor
...
/// `bsrs_native.version()` — package version string.
#[pyfunction]
...
#[pymodule]
fn bsrs_native(m: &Bound<'_, PyModule>) -> PyResult<()> {
