...
pub type SubscribeCallback = Arc<dyn Fn(&bsrs_event_model::Document) + Send + Sync + 'static>;
...
pub enum Msg {
    ...
    DeclareStream {
        ...
        data_keys: HashMap<String, bsrs_event_model::DataKey>,
    },
    ...
    /// Wait for arbitrary `Future`s — the bsrs equivalent of
    ...
    /// need to introspect whether they are running under bsrs or
    ...
    Subscribe {
        ...
        filter: bsrs_event_model::DocFilter,
    },
    ...
    /// suspender registry lives in `bsrs-engine`.
    InstallSuspender {
    ...
    Publish(Box<bsrs_event_model::Document>),
...
fn document_label(d: &bsrs_event_model::Document) -> &'static str {
...
pub trait NamedObj: Send + Sync {
    ...
    /// downstream consumers (`obj:inspect()` in Lua, `bsrs qs
...
pub trait ReadableObj: NamedObj {
    ...
    async fn read_dyn(
        &self,
    ) -> Result<HashMap<String, crate::reading::ReadingValue>, crate::error::BsrsError>;
    /// Describe each field.
    async fn describe_dyn(
        &self,
    ) -> Result<HashMap<String, bsrs_event_model::DataKey>, crate::error::BsrsError>;
...
pub trait MovableObj: NamedObj {
    ...
    async fn stop_on_pause(&self, success: bool) -> Result<(), crate::error::BsrsError> {
        let _ = success;
        Ok(())
    }
}
...
pub trait LocatableObj: MovableObj {
    ...
    async fn locate_dyn(&self) -> Result<DynLocation, crate::error::BsrsError>;
}
...
pub trait StoppableObj: NamedObj {
    ...
    async fn stop_dyn(&self, success: bool) -> Result<(), crate::error::BsrsError>;
}
...
pub trait StageableObj: NamedObj {
    /// Stage.
    async fn stage_dyn(&self) -> Result<(), crate::error::BsrsError>;
    /// Unstage.
    async fn unstage_dyn(&self) -> Result<(), crate::error::BsrsError>;
}
...
pub trait PausableObj: NamedObj {
    ...
    async fn pause_dyn(&self) -> Result<(), crate::error::BsrsError>;
    /// Called just before the engine resumes the message loop.
    async fn resume_dyn(&self) -> Result<(), crate::error::BsrsError>;
}
...
pub trait CollectableObj: NamedObj {
    ...
    async fn describe_collect_dyn(
        &self,
    ) -> Result<HashMap<String, HashMap<String, bsrs_event_model::DataKey>>, crate::error::BsrsError>;
    /// Yield events. Empty vec if nothing buffered.
    async fn collect_dyn(
        &self,
    ) -> Result<Vec<(String, HashMap<String, Value>, HashMap<String, f64>)>, crate::error::BsrsError>;
    ...
    async fn collect_stream_docs_dyn(
        &self,
        _descriptor: &str,
    ) -> Result<Vec<bsrs_event_model::Document>, crate::error::BsrsError> {
        Ok(Vec::new())
    }
}
...
pub trait MonitorableObj: ReadableObj {
    ...
    async fn subscribe_dyn(
        &self,
    ) -> Result<crate::subscription::Subscription, crate::error::BsrsError>;
}
...
pub trait ConfigurableObj: NamedObj {
    ...
    async fn configure_dyn(&self, args: ConfigureArgs) -> Result<(), crate::error::BsrsError>;
}
