...
| # | Status | Deliverable |
|---|---|---|
| M0 | ✅ done | `cirrus-event-model` + `cirrus-core` + protocols (async + sync) |
| M1 | ✅ done | `cirrus-backends/soft` + `cirrus-engine` + `count` plan + `JsonlSink` |
| M2 | ⚠️ partial | `cirrus-backends/epics-ca` real impl behind `--features real` (compiles, no live IOC test) |
...
| M5 | ✅ done | `cirrus-backends/epics-pva` real impl + `PvaMonitorSource` (NTNDArray zero-copy) + `BinaryFrameSink` |
| M6 | 🔄 in progress | Document sinks for the bluesky Python ecosystem (Zmq, Tiled, …) |
| M7 | ⏸ deferred | (optional) `cirrus-py` PyO3 plan generator |
| M8 | ⏸ planned | `cirrus-qs` queueserver worker (drop-in replacement) |
...
|---|---|---|
| **M0** | `cirrus-event-model` + `cirrus-core` + `cirrus-protocols-async` + `cirrus-protocols-sync` | Round-trip test: Document JSON → deserialize → re-serialize → equal. 11 protocol traits compile. |
| **M1** | `cirrus-backends/soft` + `cirrus-engine` + `count` plan + `JsonlSink` | `count(soft_det, 5)` emits the expected 8-document sequence. |
| **M2** | `cirrus-backends/epics-ca` (`epics-ca-rs`) | `Signal<f64, EpicsCaBackend>` connect/get/put/subscribe; sharded registry (K3); pending-Notify dedup (K4). |
...
| **M5** | `cirrus-backends/epics-pva` + `PvaMonitorSource` (NTNDArray zero-copy) | NTNDArray decode emits Frames whose `Bytes` share the PVA decode buffer's refcount (verified by unit tests). |
...
| **M7** | (deferred) `cirrus-py` PyO3 plan generator | A Python `def my_plan(): yield from ...` generator drives the cirrus RunEngine through a thin PyO3 plan adapter. |
| **M8** | **`cirrus-qs` queueserver worker** | cirrus implements the `bluesky-queueserver` worker protocol (Pipe + JSON-RPC). queueserver manager / 0MQ API / web UI / queue management unchanged. |
...
| # | Deliverable | Acceptance |
|---|---|---|
| **P2-A** | `cirrus-backends/rogue/ctrl` — ZMQ Variable backend impl `SignalBackend` | A rogue Tree Variable read/written through cirrus `Signal<T>`. K11 enforced. |
| **P2-B** | `cirrus-stream/sources/rogue_dma` impl `FrameSource` | A rogue DMA-backed detector emits Frames into the same FramePipe used by PvaMonitorSource. Plan code unchanged. |
...
- **Real Tiled write integration** — a Python relay binary (`cirrus-zmq2tiled`)
