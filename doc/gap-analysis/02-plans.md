...
bsrs-plans covers roughly 60% of the bluesky surface area.  The core compound plans
...
- **Gap:** Bluesky `count` supports inter-shot delay (scalar or per-shot iterable), a
  `per_shot` hook, and `num=None` for indefinite acquisition until canceled.  Bsrs
...
- **Gap:** Bluesky's `scan` moves all specified motors simultaneously (inner product).
  Bsrs's `scan` is 1D only.  The existing `inner_product_scan` covers the N-D case
...
- **Gap:** Bluesky `ramp_plan` polls until the ramp Status object reports completion,
  ...
  rate-limiting.  Bsrs runs exactly `samples` iterations with no completion
...
- **Gap:** Bluesky's `tune_centroid` is a multi-pass iterative algorithm.  It re-scans
  ...
  convergence.  Bsrs's version cannot converge on a peak — it makes one pass and
...
- **Gap:** Bluesky computes step size from the measured signal gradient (slope-normalized),
  providing smooth adaptation to actual local curvature.  Bsrs uses a crude halving/
  doubling rule that can oscillate around narrow features.  Backstep threshold is
  hard-coded at 1.5×/0.5× in bsrs vs configurable `threshold` in bluesky.  Bluesky
...
- **Gap:** Bluesky kicks off all flyers into one group, waits once, completes all into
  another group, waits, collects all.  Bsrs handles exactly one flyer and mixes
...
- **Gap:** Bluesky `mv` fires all motors simultaneously into one group and waits once.
  Bsrs is limited to one motor.  Multi-motor parallel moves are a daily pattern at
...
- **Gap:** Bluesky supports async group staging (`stage(obj, group="g")` → set multiple
  staging in flight → `wait(group="g")`).  Bsrs `stage` always blocks inline.
...
- **bsrs:** `preprocessors.rs:34–54` — `f: &Msg → Option<Plan>` (replace or pass-through)
...
- **Gap:** Bluesky's `plan_mutator` allows inserting messages both before and after the
  ...
  the original through.  Bsrs's `plan_mutator` can only replace a message; it cannot
...
- **bsrs:** `preprocessors.rs` — wrapper forms only
...
- **Gap:** Bluesky plan libraries use `@bpp.run_decorator(md=_md)` heavily for
  composing plan factories.  Bsrs has no decorator equivalents; callers must use
  wrappers inline.
...
- **Fix sketch:** Implement only if TUI interactive mode is added to bsrs.
- **Effort:** M
...
- **bsrs:** `preprocessors.rs:232–237` — acknowledged no-op
