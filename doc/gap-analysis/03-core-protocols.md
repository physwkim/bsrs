...
**cirrus scope:** `crates/cirrus-core/src/`, `crates/cirrus-devices/src/`,
`crates/cirrus-protocols-async/src/`, `crates/cirrus-protocols-sync/src/`,
`crates/cirrus-derive/src/`
...
**cirrus:** `crates/cirrus-devices/src/signal.rs:34` — `Signal<T, B>` is a
...
Without this split cirrus cannot express "this PV is write-only" or "this
...
**cirrus:** No SignalX anywhere in `crates/cirrus-*`.
...
implements the `Triggerable` protocol trait (from `cirrus-protocols-async`) by
...
**cirrus:** `crates/cirrus-core/src/kind.rs:6` has `Kind { Normal, Config,
Hinted, Omitted }` for document routing.  `crates/cirrus-devices/src/signal.rs:
...
**Fix sketch:** Add `StandardReadable` struct in `cirrus-devices` that holds
...
**cirrus:** No `MockSignalBackend`, no `SoftSignalBackend`, no `connect_all(mock=true)`,
no `set_mock_value`, no `get_mock_put`, no `callback_on_mock_put`.
`crates/cirrus-core/`, `crates/cirrus-devices/`, `crates/cirrus-protocols-async/`
...
**Fix sketch:** (a) Add `SoftSignalBackend<T>` to `cirrus-devices`: an in-process
...
**cirrus:** `crates/cirrus-core/src/status.rs:44` — `Inner.progress:
...
**Fix sketch:** Add `WatcherUpdate<T>` struct to `cirrus-core` and a
...
Also add `Watcher` as a trait or function-pointer type to `cirrus-protocols-async`.
...
**cirrus:** `#[derive(Device)]` (`crates/cirrus-derive/src/lib.rs:36`) generates
...
**cirrus:** No DeviceVector anywhere in cirrus.
...
**Fix sketch:** Add `DeviceVector<T>` struct in `cirrus-devices` implementing
...
**cirrus:** `crates/cirrus-devices/src/signal.rs:84` — `Signal::read()` always
...
**Fix sketch:** Add `SignalCache<T>` in `cirrus-devices` wrapping the existing
...
**cirrus:** No equivalent of `observe_value`, `observe_signals_value`, or
`wait_for_value` anywhere in cirrus.  Users can manually watch a `Subscription`
...
as standalone async functions in `cirrus-devices` or a new `cirrus-plans` module.
...
**cirrus:** No in-process, non-I/O `SignalBackend<T>` implementation.  Building an
...
**Fix sketch:** `SoftSignalBackend<T>` in `cirrus-devices`: holds
...
**cirrus:** `crates/cirrus-protocols-async/src/lib.rs:38` —
...
**cirrus:** `crates/cirrus-core/src/msg.rs:562` — `ReadableObj::hint_fields() ->
...
**Fix sketch:** Add `pub struct Hints { pub fields: Vec<String> }` to `cirrus-core`
and `trait HasHints { fn hints(&self) -> Hints; }` to `cirrus-protocols-async`.
...
**cirrus:** `DataKey` construction is entirely left to each backend implementation.
...
precision: Option<u8>, units: Option<String> }` to `cirrus-core` and a
`fn make_datakey(source: &str, dtype: Dtype, shape: Vec<usize>, meta: SignalMetadata) -> DataKey`
constructor in `cirrus-event-model` or `cirrus-devices`.
...
**cirrus:** `crates/cirrus-core/src/status.rs` — `Status` is a `Future` returning
...
**Gap:** Cirrus `Status` cannot signal cancellation to the device that generated it;
...
**cirrus:** No signal that computes its value as a transformation of other signals.
...
**cirrus:** `crates/cirrus-protocols-async/src/lib.rs:50` —
...
**cirrus:** No factory functions for soft signals.
...
**cirrus:** `crates/cirrus-devices/src/signal.rs:73` — `Signal::put()` makes one
...
**cirrus:** No parallel-connect + auto-name context manager.
...
**cirrus:** No device-tree traversal utilities.
...
**Fix sketch:** Add to `cirrus-devices` after CP-06 (`children()` iterator): a
...
- `Msg` enum (`crates/cirrus-core/src/msg.rs`) is comprehensive and closely tracks
...
- `Status` (`cirrus-core/status.rs`) covers `done`, `success`, `exception`,
  `progress`, `add_callback`, sync `wait(timeout)` — functional parity with
  `bluesky.protocols.Status` for the common cases.
- `Location<T>` / `locate()` exist as `Locatable<T>` in `cirrus-protocols-async`
...
- `cirrus-protocols-sync` blanket-impls sync wrappers via `block_on` — matches
